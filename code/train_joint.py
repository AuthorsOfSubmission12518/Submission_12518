...
from whole_net.joint_net import Joint_Net, Joint_Net_LIC
from com_net.util.loss import RateDistortionLoss, RDDLoss

from pms.joint.recon_cheng128 import pms
from folder.spk_folder import spk_provider, spk_provider_fake
    ...
    for i, d in enumerate(train_dataloader):
        ...
        if round(100. * i / len(train_dataloader), 1) > cur_schedule:
            ...
            if round(100. * i / len(train_dataloader), 1) > cur_save_schedule:
                ...
                file_name = f"{pms['save_checkpoint']}/temp/{pms['lmbda']}_lastest.pth"
                torch.save(
                    {
                        "epoch": epoch,
                        'i': i,
...
def test_one_epoch(
    ...
    # if os.path.exists(f"{pms['save_checkpoint']}/temp/{pms['lmbda']}_lastest.pth"):
    #     os.remove(f"{pms['save_checkpoint']}/temp/{pms['lmbda']}_lastest.pth")
...
def main():
    ...
    # train_dataset = spk_provider_fake()
    # test_dataset = spk_provider_fake()
    ...
    test_dataloader = DataLoader(
        ...
        num_workers=pms['test_batch'] // 4,
        shuffle=False,
        pin_memory=(pms['device'] != "cpu")
    )

    net = Joint_Net_LIC(pms['recon_net_path'], pms['com_net_path'], pms['device'])
    if pms['recon_detach']:
        for p in net.recon_net.parameters():
            p.requires_grad_(False)
    net = net.to(pms['device'])
    optimizer = create_optimizers(net, pms['learning_rate'])
    lr_scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode="min", patience=5, factor=.1)
    if pms['rdd']:
    ...
    best_loss = float("inf")
    best_iteration = 0
    if pms['load_checkpoint'] is not None:
        try:
            checkpoint = torch.load(pms['load_checkpoint'], map_location=pms['device'])
        ...
        net.com_net.load_state_dict(
            torch.load(f'/home/kxfeng/iccv/checkpoint/baseline/lic_{pms["lmbda"]}checkpoint_best.pth.tar',
    ...
    for epoch in range(last_epoch, pms['epochs']):
        ...
        if loss < best_loss:
            ...
            if pms['save']:
                ...
                file_name = f"{pms['save_checkpoint']}/{pms['lmbda']}_best.pth"
                torch.save(
                    {
                        "epoch": epoch,
...
# parser.add_argument('--lmbda', type=int, default=0.05)
# parser.add_argument('--device', type=str, default='cuda:6')
# parser.add_argument('--batch_size', default=3, type=int)
# parser.add_argument('--worker_num', default=1, type=int)
# parser.add_argument('--inference', action='store_true', default=False)
# parser.add_argument('--recon_detach', action='store_true', default=False)
# parser.add_argument('--rdd', action='store_true', default=True)
# args = parser.parse_args()
...
if __name__ == '__main__':
    ...
    # pms['load_checkpoint'] = f'/home/kxfeng/iccv/checkpoint/lic/joint-{args.lmbda}.pth'
    pms['load_checkpoint'] = None
    pms['recon_net_path'] = '/home/kxfeng/iccv/checkpoint/recon_net.pth'
    pms['com_net_path'] = f'/home/kxfeng/iccv/checkpoint/baseline/{pms["lmbda"]}checkpoint_best.pth.tar.pth'
    pms['rdd'] = args.rdd
    pms['recon_detach'] = args.recon_detach
    pms['save_checkpoint'] = '/home/kxfeng/iccv/checkpoint/lic'
